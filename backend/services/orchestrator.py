...
class AgentOrchestrator:
    ...
    async def route_intent(self, user_message: str, history_msgs: list[dict] = None) -> str:
        ...
        2. BIENESTAR: Si habla de su estado físico o mental (sueño, cansancio, estrés, fatiga, hábitos de descanso, cómo se siente emocionalmente) o responde a preguntas sobre su estado de ánimo/descanso.
           TAMBIÉN es BIENESTAR si la conversación reciente trata sobre evaluar o recoger el informe de una sesión de estudio que se acaba de parar... INCLUSO si la respuesta del usuario menciona incidentalmente asignaturas o tareas al explicar qué hizo.
           EXCEPCIÓN CRÍTICA: Si el usuario IGNORA la pregunta de cómo le fue la sesión y en su lugar te pide EXPLÍCITAMENTE realizar una acción académica (como "muéstrame mis tareas", "¿qué tareas tengo?", "añade esta tarea"), debes clasificarlo SIEMPRE como ACADEMICO, porque el usuario ha cambiado de tema.
